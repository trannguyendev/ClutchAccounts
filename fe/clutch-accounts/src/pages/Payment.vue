...
<template>
    <div class="min-h-[70vh] flex items-start justify-center py-12 px-4 bg-cover bg-center bg-no-repeat"
        ...
        <div
            ...
            <h2 class="text-2xl font-extrabold text-amber-300 mb-4">{{ t('payment.addMoney') }}</h2>
            <p class="text-sm text-amber-200/80 mb-6">{{ t('payment.enterAmount') }}</p>

            <div class="space-y-4">
                <label class="block">
                    <span class="text-amber-200 text-sm font-semibold">{{ t('payment.amount') }}</span>
                    <div class="mt-2 flex items-center gap-3">
                        ...
                        <button @click="confirmPayment" :disabled="loading"
                            ...
                            <span v-if="!loading">{{ t('payment.confirmPayment') }}</span>
                            <span v-else>{{ t('payment.generatedQR') }}</span>
                        </button>
                    </div>
                </label>
                ...
                <div v-if="qrVisible"
                    ...
                    <div class="w-full flex justify-between items-start mb-3">
                        <div>
                            <div class="text-amber-300 font-bold">{{ t('payment.scanToPay') }}</div>
                            <div class="text-amber-200 text-sm">{{ t('payment.paymentAmount') }}: <span class="font-semibold">{{ amount }}</span>
                            </div>
                        </div>
                    ...
                    <div class="bg-black/80 p-4 rounded-lg">
                        ...
                        <div v-if="loading" class="w-64 h-64 flex items-center justify-center">
                            <div class="space-y-4 w-full px-4">
                                ...
                                <p class="text-center text-amber-300 text-sm font-semibold">{{ t('common.loading') }}...</p>
                            </div>
                        </div>
                        <!-- QR image -->
                    ...
                    <button @click="checkPaymentStatus" :disabled="requestLoading"
                        ...
                        <span v-if="!requestLoading">{{ t('payment.confirmPayment') }}</span>
                ...
                <div v-else class="mt-6 text-amber-300/70 text-sm">{{ t('payment.confirmPayment') }} {{ t('common.confirm') }}.</div>
                <button @click="goBack"
        ...
        <div
            ...
            <div class="flex items-center gap-3 mb-6">
                ...
                <h3 class="text-2xl font-extrabold text-red-300">{{ t('payment.importantNotes') }}</h3>
            </div>
            ...
            <div class="space-y-4 text-red-100/90 text-sm leading-relaxed">
                <div class="p-3 rounded-lg hover:bg-red-800/30 transition duration-200 cursor-default">
                    <span class="font-semibold text-red-300">{{ t('payment.notes.attention').split(':')[0] }}:</span> {{ t('payment.notes.attention').split(':')[1] }}
                </div>

                <div class="p-3 rounded-lg hover:bg-red-800/30 transition duration-200 cursor-default">
                    <span class="font-semibold text-red-300">{{ t('payment.notes.minimumAmount').split(':')[0] }}:</span> {{ t('payment.notes.minimumAmount').split(':')[1] }}
                </div>

                <div class="p-3 rounded-lg hover:bg-red-800/30 transition duration-200 cursor-default">
                    <span class="font-semibold text-red-300">{{ t('payment.notes.transactionContent').split(':')[0] }}:</span> {{ t('payment.notes.transactionContent').split(':')[1] }}
                </div>

                <div class="p-3 rounded-lg hover:bg-red-800/30 transition duration-200 cursor-default">
                    <span class="font-semibold text-red-300">{{ t('payment.notes.supportedWallets').split(':')[0] }}:</span> {{ t('payment.notes.supportedWallets').split(':')[1] }}
                </div>

                <div class="p-3 rounded-lg hover:bg-red-800/30 transition duration-200 cursor-default">
                    <span class="font-semibold text-red-300">{{ t('payment.notes.verification').split(':')[0] }}:</span> {{ t('payment.notes.verification').split(':')[1] }}
                </div>

                <div class="p-3 rounded-lg hover:bg-red-800/30 transition duration-200 cursor-default">
                    <span class="font-semibold text-red-300">{{ t('payment.notes.processingTime').split(':')[0] }}:</span> {{ t('payment.notes.processingTime').split(':')[1] }}
                </div>

                <div class="p-3 rounded-lg hover:bg-red-800/30 transition duration-200 cursor-default">
                    <span class="font-semibold text-red-300">{{ t('payment.notes.errorHandling').split(':')[0] }}:</span> {{ t('payment.notes.errorHandling').split(':')[1] }}
                </div>

                <div class="p-3 rounded-lg hover:bg-red-800/30 transition duration-200 cursor-default">
                    <span class="font-semibold text-red-300">{{ t('payment.notes.support').split(':')[0] }}:</span> {{ t('payment.notes.support').split(':')[1] }}
                </div>
            </div>
        </div>
    </div>
